"""XSS Payload Tester - desktop GUI (Tkinter, portable, no install)."""
...
from app.core.framemap import MODULES
...
COLS = ("sev", "verdict", "category", "vector", "context", "strategy", "cvss", "url", "param", "conf")
...
class App(tk.Tk):
    ...
    def _build_target_tab(self) -> ttk.Frame:
        ...
        row = 0
        ttk.Label(f, text="Target URL (your lab app, e.g. http://127.0.0.1:8080/)").grid(row=row, column=0, sticky="w", pady=4)
        ...
        ttk.Label(f, text="Max payloads/category").grid(row=row, column=2, sticky="e", pady=4)
        self.v_maxpay = tk.StringVar(value="400")
        ttk.Spinbox(f, from_=10, to=5000, textvariable=self.v_maxpay, width=10).grid(row=row, column=3, sticky="w", pady=4)
        row += 1

        ttk.Label(f, text="Max requests").grid(row=row, column=0, sticky="w", pady=4)
        self.v_maxreq = tk.StringVar(value="1500")
        ttk.Spinbox(f, from_=10, to=100000, textvariable=self.v_maxreq, width=10).grid(row=row, column=1, sticky="w", pady=4)
        ttk.Label(f, text="Concurrency (threads)").grid(row=row, column=2, sticky="e", pady=4)
        self.v_conc = tk.StringVar(value="4")
        ttk.Spinbox(f, from_=1, to=16, textvariable=self.v_conc, width=8).grid(row=row, column=3, sticky="w", pady=4)
        row += 1

        ttk.Label(f, text="Politeness delay (ms)").grid(row=row, column=0, sticky="w", pady=4)
        self.v_delay = tk.StringVar(value="100")
        ttk.Spinbox(f, from_=0, to=5000, increment=25, textvariable=self.v_delay, width=10).grid(row=row, column=1, sticky="w", pady=4)
        ttk.Label(f, text="HTTP timeout (s)").grid(row=row, column=2, sticky="e", pady=4)
        self.v_timeout = tk.StringVar(value="15")
        ttk.Spinbox(f, from_=1, to=120, textvariable=self.v_timeout, width=8).grid(row=row, column=3, sticky="w", pady=4)
        row += 1

        ttk.Label(f, text="Proxy (http://host:port)").grid(row=row, column=0, sticky="w", pady=4)
        self.v_proxy = tk.StringVar()
        ttk.Entry(f, textvariable=self.v_proxy, width=28).grid(row=row, column=1, sticky="w", pady=4)
        ttk.Label(f, text="Seed").grid(row=row, column=2, sticky="e", pady=4)
        self.v_seed = tk.StringVar(value="20260920")
    ...
    def _build_payloads_tab(self) -> ttk.Frame:
        ...
        top = ttk.Frame(f)
        top.pack(fill="x")
        ttk.Label(top, text="Select XSS payload categories (mapped to OWASP Top 10:2025)").pack(side="left", pady=6)
        ...
        for i, (key, meta) in enumerate(MODULES.items()):
            ...
            cb = ttk.Checkbutton(wrap, text=f"{key.upper():15s} {str(meta['title'])}", variable=var)
            cb.grid(row=i, column=0, sticky="w", padx=6, pady=3)
            ttk.Label(wrap, text=f"→ {str(meta['owasp'])} | CWE {'/'.join(str(m) for m in meta['cwes'])}", foreground="#555").grid(row=i, column=1, sticky="w", padx=2)
    ...
    def _build_findings_tab(self) -> ttk.Frame:
        ...
        for c, (label, w) in heads.items():
            ...
            self.tree.column(c, width=w, stretch=(c in ("url",)))
    ...
    def _build_report_tab(self) -> ttk.Frame:
        ...
        ttk.Label(f, text="Verdicts are response-reflection triage evidence — confirm PoCs in a browser.", foreground="#555").pack(anchor="w", pady=2)
        row = ttk.Frame(f)
    ...
    def _config_values(self) -> dict:
        try:
            return {
                ...
                "max_payloads": max(10, min(5000, int(self.v_maxpay.get()))),
            ...
            messagebox.showerror("Invalid value", "Check numeric fields (pages, payloads, requests, concurrency, delay, timeout, seed).")
            return {}

    def _start_scan(self) -> None:
        if not self.v_url.get().strip():
            messagebox.showwarning("No target", "Enter a target URL (your lab app) to scan.")
            return
        ...
        if not cfg.modules:
            messagebox.showwarning("No modules", "Select at least one payload category.")
            return
        ...
        self._write_summary(
            ...
            f"Target: {cfg.url}\nCategories: {', '.join(cfg.modules)}\n"
            f"Audit: {AUDIT_DIR / 'audit.jsonl'}\n"
        )
    ...
    def _scan_finished(self) -> None:
        ...
        self._write_summary(
            ...
            + "Verdicts are triage evidence; confirm each PoC URL in a browser before remediation."
        )
    ...
    def _tree_insert(self, f: Finding) -> None:
        self.tree.insert(
            ...
            "end",
            iid=f.id,
            values=(f.severity, f.verdict, f.module, f.vector_name, f.context, f.strategy, f"{f.cvss_score:.1f}", f.url, f.param, f"{f.confidence:.2f}"),
        )
    ...
    def _show_detail(self, f: Finding) -> None:
        ...
        text = (
            f"{f.id} — {f.title} [{f.severity}] verdict={f.verdict} conf={f.confidence:.2f}\n"
            f"URL: {f.url}\nParameter: {f.param} | Vector: {f.vector_name} | Context: {f.context} | Strategy: {f.strategy}\n"
            f"CVSS v3.1: {f.cvss_score:.1f} ({f.cvss_vector})\n"
    ...
    def _apply_config(self, cfg: dict) -> None:
        self._vset(cfg, "url", self.v_url)
        self._vset(cfg, "auth_cookie", self.v_cookie)
        self._vset(cfg, "auth_header", self.v_authhdr)
        self._vset(cfg, "proxy", self.v_proxy)
        self._vset(cfg, "seed", self.v_seed)
        self._vset(cfg, "max_pages", self.v_pages)
        self._vset(cfg, "max_payloads", self.v_maxpay)
        self._vset(cfg, "max_requests", self.v_maxreq)
        self._vset(cfg, "concurrency", self.v_conc)
        self._vset(cfg, "delay_ms", self.v_delay)
        self._vset(cfg, "timeout", self.v_timeout)
        txt = cfg.get("extra_headers")
        if txt:
            self.v_extrahdrs.delete("1.0", "end")
            self.v_extrahdrs.insert("1.0", str(txt))
        txt = cfg.get("notes")
        if txt:
            self.v_notes.delete("1.0", "end")
            self.v_notes.insert("1.0", str(txt))
    ...
    def _vset(self, cfg: dict, key: str, var) -> None:
        if key in cfg:
            var.set(str(cfg[key]))
    ...
    def _about(self) -> None:
        messagebox.showinfo(
            "About",
            f"{APP} v1.0\n\nA portable XSS payload tester for your own lab web application.\n"
            "Maps findings to OWASP Top 10:2025, NIST SP 800-53/800-218, ISO/IEC 27001:2022 Annex A.\n"
            "Response-reflection triage: confirm each PoC in a browser before remediating.\n\n"
