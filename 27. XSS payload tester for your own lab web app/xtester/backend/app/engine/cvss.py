...
Used so findings carry an industry-consistent severity (OWASP guidance,
ISO 27001 A.8.8 risk classification). Defaults model reflected XSS:
...
class Cvss:
    vector: str = field(
        default="CVSS:3.1/AV:N/AC:L/PR:N/UI:R/S:C/C:L/I:L/A:N"
    )
    ...
    def base_score(self, exploitability=None, impact=None) -> float:
        m = self.parse()
...
def rate(executed: bool, confidence: float, context: str) -> tuple[str, float]:
    """Best-effort single number: (severity, cvss_score) for reported context."""
    cvss = Cvss.for_context(context, executed)
    return cvss.severity(), cvss.base_score()