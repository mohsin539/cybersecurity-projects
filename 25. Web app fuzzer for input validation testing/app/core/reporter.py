"""Web App Fuzzer - report export (JSON + HTML with traceability records)."""
...
def render_html(findings: List[Finding], meta: Dict) -> str:
    ...
    for f in sorted(findings, key=lambda f: SEVERITY_WEIGHT.get(f.severity, 9)):
        ...
        rows.append(
            ...
            f"<td>{html.escape(f.payload_id)}</td>"
            f"<td>{f.confidence:.2f}</td>"
            f"<td>{html.escape(ev)}</td>"
            f"<td>{html.escape(f.remediation[:120])}</td>"
            "</tr>"
        )
    meta_rows = "".join(
    ...
    page = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><title>Web App Fuzzer Report</title>
<style>
body{{font-family:Segoe UI,Arial,sans-serif;margin:2em;color:#1a1a2e}}
h1{{font-size:1.4em}} table{{border-collapse:collapse;width:100%;font-size:0.8em}}
...
<table>
<tr><th>ID</th><th>Severity</th><th>Title</th><th>OWASP 2025</th><th>CWE</th>
<th>NIST 800-53</th><th>ISO 27001</th><th>URL</th><th>Param</th><th>Payload</th>
<th>Conf</th><th>Evidence</th><th>Remediation</th></tr>
{''.join(rows)}
</table>
</body></html>"""
    return page
