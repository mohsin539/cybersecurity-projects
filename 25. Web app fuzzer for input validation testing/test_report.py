...
with TemporaryDirectory() as tmp:
    ...
    cfg = ScanConfig(url=url, modules=["xss", "sqli", "traversal", "errors"], max_pages=10, max_requests=150, concurrency=2, delay_ms=0)
    found = []
    eng = Engine(cfg, on_finding=found.append, audit_dir=audit)
    found = eng.run()
    print("findings:", len(found))
    meta = {"target": url, "modules": "xss,sqli,traversal,errors", "requests": eng.requests, "findings": len(found)}
